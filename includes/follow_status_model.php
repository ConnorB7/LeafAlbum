...
function check_follow_status(object $pdo, string $user_id, int $to_follow_id){
...
function get_user_id_from_username($pdo, string $username){
...
function user_is_following($pdo, $user_id, $followable_id){
	$query = "SELECT FROM followings WHERE user_id = :user_id AND following_id = :following_id;";
	$stmt = $pdo->prepare($query);
	$stmt->bindParam(":user_id", $followable_id);
    $stmt->bindParam(":following_id", $user_id);
	$stmt->execute();
    if($result){
        return "following";
